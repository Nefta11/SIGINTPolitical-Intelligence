...
function getApiUrl() {
  if (API_MODE === 'proxy') {
    return `http://localhost:${PROXY_PORT}/api/claude`;
  }
...
// Función para calcular delay exponencial más agresivo
function calculateBackoffDelay(attempt, baseDelay = 5000) {
  // Delays más largos: 5s, 15s, 45s, 90s, 180s
...
export async function callClaude(systemPrompt, userPrompt, useSearch = false, retryCount = 0, useSmallModel = false) {
  if (!API_KEY && API_MODE === 'direct') {
    throw new Error('API key no configurada. Edita el archivo .env y agrega tu VITE_ANTHROPIC_API_KEY');
  }
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      // Manejar rate limit (429) con retry más agresivo
      if (resp.status === 429 && retryCount < 5) { // Aumentado a 5 reintentos
        const delay = calculateBackoffDelay(retryCount + 1);
        console.log(`⏳ Rate limit - Reintento ${retryCount + 1}/5 en ${delay/1000}s... ${useSmallModel ? '(modelo pequeño)' : ''}`);
        await sleep(delay);
        
        // Cambiar a modelo pequeño después de 2 reintentos fallidos
        const shouldUseSmallModel = retryCount >= 1 || useSmallModel;
        return callClaude(systemPrompt, userPrompt, false, retryCount + 1, shouldUseSmallModel); // Sin search en reintentos
      }
    ...
    // Si es un error de rate limit pero ya agotamos los reintentos
    if (error.message.includes('429') && retryCount >= 5) {
      throw new Error(`Rate limit persistente después de ${retryCount} reintentos. El sistema necesita una pausa más larga - intenta en 10-15 minutos o usa el modo de espera automática.`);
    }
    throw error;
  }
}
